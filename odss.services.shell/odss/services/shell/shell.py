...
class Shell:
    ...
    def print_help(self, session: OutputStream, name: str = None) -> None:
        ...
        if name:
            ...
            namespaces.remove(SHELL_DEFAULT_NAMESPACE)
            namespaces.sort()
            namespaces.append(SHELL_DEFAULT_NAMESPACE)
    ...
    def bind_handler(self, handler: t.Any):
        ...
        for method, attrs in _extract_command_handlers(handler):
            name = attrs["name"]
            namespace = attrs.get("namespace") or SHELL_DEFAULT_NAMESPACE
    ...
    def register_command(
        self, name: str, handler: t.Callable, namespace: str = SHELL_DEFAULT_NAMESPACE
    ) -> bool:
        ...
        lname = name if namespace == SHELL_DEFAULT_NAMESPACE else f"{namespace}.{name}"
    ...
    def unregister_command(
        self, name: str, namespace: str = SHELL_DEFAULT_NAMESPACE
    ) -> bool:
        name = (name or "").strip().lower()
        ...
        if namespace in self._commands and name in self._commands[namespace]:
            ...
            lname = (
                name if namespace == SHELL_DEFAULT_NAMESPACE else f"{namespace}.{name}"
            )
    ...
    def get_commands(self, namespace: str = SHELL_DEFAULT_NAMESPACE):
        try:
    ...
    def get_all_commands(self):
        ...
        for namespace, handlers in self._commands.items():
            for name in handlers.keys():
                if namespace == SHELL_DEFAULT_NAMESPACE:
...
def _split_command_name(
    name: str, default_namespace=SHELL_DEFAULT_NAMESPACE
) -> t.Tuple[str, str]:
    parts = name.split(".")
    if len(parts) == 1:
...
def _extract_command_handlers(obj: t.Any) -> HandlerInfo:
    ...
    for name, fn in members:
        if not name.startswith("_"):
            attrs = getattr(fn, SHELL_COMMAND_HANDLER, None)
            if attrs:
                yield fn, attrs
