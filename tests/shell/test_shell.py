...
def test_registration_command(shell):
    ...
    assert not shell.register_command('test', command_handler), "Command registered twice"
...
async def test_execute_incorrect_commands(shell, shell_session):
    ...
    assert not await shell.execute('test', shell_session)
    assert shell_session.output == "Command call problem: 'int' object is not callable\n"
