...
from odss.common import IBundle, IBundleContext, get_class_name
...
class Requirement:
    ...
    def __hash__(self):
        return hash(self.specification + self.spec_filter + str(self.reference))

    def __repr__(self) -> str:
        return f"Requirement[specification={self.specification} ref={self.reference} filter={self.spec_filter}]"
...
def get_method_spec(method: t.Callable):
    ...
    for pname, pvalue in sig.parameters.items():
        if pname != "self" and pvalue.annotation != sig.empty:
            yield get_class_name(pvalue.annotation)
...
class FactoryContext:
    ...
    def set_bundle(self, bundle: IBundle):
        self.__bundle = bundle

    def get_bundle(self) -> IBundle:
        return self.__bundle

    def get_bundle_context(self) -> IBundleContext:
